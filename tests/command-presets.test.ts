...
test('normalizes legacy steward commands into conversation and augment lists', () => {
    const steward: any = {
        ...
        commands: [
            { id: 'summary', name: '旧摘要', icon: '🪄', detailPrompt: '', type: 'default-summary' },
            { id: 'chat', name: '旧对话', icon: '💬', detailPrompt: '', type: 'annotated' },
        ],
    };
    ...
    assert.equal(steward.commands.some((command: any) => command.type === 'default-summary'), false);
    assert.equal(steward.commands.every((command: any) => command.type === 'conversation'), true);
    assert.equal(steward.augmentCommands?.every((command: any) => command.type === 'augment'), true);
    assert.equal(steward.commands.every((command: any) => command.enabled === true), true);
});
...
test('fills inline commands to six and keeps the eight-command ceiling', () => {
    const steward: any = {
        ...
        temperature: 0.3,
        commands: [
            { id: 'inline', name: '旧改写', icon: '✏️', detailPrompt: '', type: 'inline-modify' },
        ],
    };
    ...
    assert.equal(steward.commands.length, 6);
    assert.equal(steward.commands.every((command: any) => command.type === 'inline-modify'), true);
