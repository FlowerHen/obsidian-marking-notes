...
export interface PopoverContext {
    app: App;
    onFollowUp: (nodeId: string, instruction: string, currentContent: string, options?: { enableWebSearch?: boolean }) => Promise<{ summary: string; richText: string } | null>;
...
export class PopoverEditor {
    ...
    async show(anchorX: number, anchorY: number) {
        ...
        const toggleBtn = this.createCtrlBtn('📝', '编辑模式', () => this.toggleMode());
    ...
    private updateTitleDisplay() {
        if (!this.container) return;
        const titleArea = this.container.querySelector('#ai-popover-title-display') as HTMLElement;
        if (!titleArea) return;
        titleArea.empty();

        const displayText = this.node.summary
            ? (this.node.summary.length > 24 ? this.node.summary.slice(0, 24) + '…' : this.node.summary)
            : (this.node.id || '无标题');
    ...
    private createCtrlBtn(text: string, tooltip: string, onclick: () => void): HTMLButtonElement {
    ...
    private async renderMarkdownView() {
        ...
        try {
            ...
            await (MarkdownRenderer as any).render(
                this.ctx.app, this.currentContent, this.viewContainer, '', this.renderComponent
            );
        } catch {
            try {
                await (MarkdownRenderer as any).renderMarkdown(
                    this.currentContent, this.viewContainer, '', this.renderComponent
                );
            } catch {
...
export class PopoverViewer {
    ...
    private currentParams: { nodeId: string, nodeSummary: string, nodeState: string, nodeTagId: string } | null = null;
    ...
    async show(nodeId: string, nodeSummary: string, nodeState: string, nodeTagId: string, richText: string, anchorX: number, anchorY: number) {
        this.close();
        ...
        if (richText) {
            try {
                await (MarkdownRenderer as any).render(
                    this.ctx.app, richText, viewContainer, '', this.renderComponent
                );
            } catch {
                const pre = document.createElement('pre');
                pre.style.whiteSpace = 'pre-wrap';
    ...
    private updateTitleDisplay() {
        ...
        const titleArea = this.container.querySelector('#ai-viewer-title-display') as HTMLElement;
        if (!titleArea) return;
        titleArea.empty();

        const p = this.currentParams;
        const displayText = p.nodeSummary
            ? (p.nodeSummary.length > 24 ? p.nodeSummary.slice(0, 24) + '…' : p.nodeSummary)
            : (p.nodeId || '阅读视图');
    ...
    private jumpToSource(nodeId: string) {
        ...
        for (const el of Array.from(els)) {
            ...
            if ((el as HTMLElement).dataset.tagId || (el as HTMLElement).innerText.includes(nodeId) || (el as HTMLElement).dataset.markingId === nodeId) {
    ...
    private makeDraggable(handle: HTMLElement) {
        ...
        const onMouseUp = () => { isDragging = false; handle.style.cursor = 'grab'; };
...
export class ButlerFloatingPanel {
    ...
    show() {
        ...
        if (this.plugin.settings.enableDebugMode) {
            console.log('[Marking Note] Butler panel shown, stewards:', this.plugin.settings.stewards?.length);
        }
    ...
    private refresh() {
        ...
        const steward = stewards.find((s: any) => s.id === settings.activeStewardId) || stewards[0];
    ...
    private setupDrag(handle: HTMLElement) {
        ...
        document.addEventListener('touchmove', (e: TouchEvent) => {
            if (!this.isDragging || !this.container || e.touches.length === 0) return;
            moveDrag(e.touches[0].clientX, e.touches[0].clientY);
            if (e.cancelable) e.preventDefault();
        }, { passive: false });
