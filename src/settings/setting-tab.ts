...
export class MarkingNoteSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const switchTab = (tab: SettingsTabKey) => {
			this.activeTab = tab;
			for (const [key, el] of Object.entries(sectionEls) as Array<
				[SettingsTabKey, HTMLDivElement]
			>) {
				el.style.display = key === tab ? "" : "none";
			}
			for (const [key, btn] of Object.entries(tabButtons) as Array<
				[SettingsTabKey, HTMLButtonElement | undefined]
			>) {
	...
	private renderGeneralSettings(container: HTMLElement) {
		...
		generalHint.createEl("div", {
			text: `${UI_ICONS.actionChat} 悬浮菜单说明`,
	...
	private renderModelSettings(container: HTMLElement) {
		container.createEl("h3", {
			text: `${UI_ICONS.models} 模型提供商 (Model Providers)`,
			cls: "mn-settings-section-title",
		});
		container.createEl("p", {
		...
		for (
			...
			new Setting(providerDiv).setName("名称").addText((text) =>
				text.setValue(provider.name).onChange(async (value) => {
					this.plugin.settings.modelProviders[index].name = value;
					await this.plugin.saveSettings();
				}),
			);
			new Setting(providerDiv).setName("Base URL").addText((text) =>
				text
			...
			new Setting(providerDiv).setName("Model ID").addText((text) =>
				text
	...
	private renderStewardSettings(
		container: HTMLElement,
		switchTab: (tab: SettingsTabKey) => void,
	) {
		container.createEl("h3", {
			text: `${UI_ICONS.steward} 切换当前管家`,
			cls: "mn-settings-section-title",
		});
		container.createEl("p", {
	...
	private renderInlineStewardSettings(container: HTMLElement) {
		container.createEl("h3", {
			text: `${UI_ICONS.actionRewrite} 全局改写管家 (Inline Steward)`,
			cls: "mn-settings-section-title",
		...
		new Setting(inlineDiv).setName("识别上下文长度界限").addSlider((slider) =>
			slider
		...
		inlineCommands.forEach((cmd, index) => {
			...
			const cardHeader = cmdCard.createEl("div", {
				cls: "mn-command-card-header",
			});
			cardHeader.createEl("div", {
				text: `${cmd.icon} ${cmd.name}`,
				attr: { style: "font-weight: 600;" },
			});
			const controls = cardHeader.createEl("div", {
				cls: "mn-command-card-controls",
			});
			...
			const cmdDel = controls.createEl("button", {
				text: "✖",
				attr: { title: "删除指令" },
			});
			...
			const descPreview =
				cmd.detailPrompt.slice(0, 60) +
				(cmd.detailPrompt.length > 60 ? "..." : "");
			cmdCard.createEl("div", {
				text: descPreview || "未设置提示词",
				cls: "mn-command-preview",
			});
			cmdCard.onclick = () => {
	...
	private renderEditableStewardSettings(
		container: HTMLElement,
		switchTab: (tab: SettingsTabKey) => void,
	) {
		container.createEl("h3", {
			text: `${UI_ICONS.steward} 管家维护与配置`,
			cls: "mn-settings-section-title",
			attr: { style: "margin-top: 30px;" },
		});
		container.createEl("p", {
		...
		const index = this.plugin.settings.stewards.findIndex(
			(steward) => steward.id === this.editingStewardId,
		);
		if (index === -1) return;
		...
		new Setting(stewardDiv).setName("名称").addText((text) =>
		...
		new Setting(stewardDiv).setName("图标 (Emoji)").addButton((btn) =>
		...
		new Setting(stewardDiv)
			.setName("阅读理解提示词")
			.setDesc('定义该管家如何"看"待文本。如：总结核心、分析技术点。')
			.addTextArea((text) =>
		...
		new Setting(stewardDiv)
			.setName("写作风格提示词")
			.setDesc("定义大模型返回结果（富文本区域）的语气与排版特征。")
			.addTextArea((text) =>
		...
		new Setting(stewardDiv).setName("Temperature (温度)").addSlider((slider) =>
			slider
				.setLimits(0, 1, 0.1)
		...
		new Setting(stewardDiv).setName("TopP").addSlider((slider) =>
			slider
				.setLimits(0, 1, 0.05)
		...
		const tabButtons: Partial<
			Record<"conversation" | "augment", HTMLButtonElement>
		> = {};
		...
		for (const [category, label] of [
			["conversation", `${UI_ICONS.actionChat} 对话指令`],
			["augment", `${UI_ICONS.actionAugment} 增补指令`],
		] as const) {
		...
		const renderCommandList = (category: "conversation" | "augment") => {
			...
			commands.forEach((cmd, cmdIndex) => {
				...
				const checkbox = rightControls.createEl("input", {
					attr: { type: "checkbox" },
				});
				checkbox.checked = cmd.enabled !== false;
