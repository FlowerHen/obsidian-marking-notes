import type { InlineStewardConfig, LightningCommand, StewardConfig } from '../domain/types';
...
const CONVERSATION_PRESETS = [
    ...
    ['联系上下文', '🔗', '说明选中文本与上下文的关系，以及它在整体内容中的作用。'],
...
function normalizeCommand(command: LightningCommand, type: 'conversation' | 'augment'): LightningCommand {
    return {
        ...command,
        type,
...
function fillCommands(
    ...
    for (const command of commands) {
        if (!byId.has(command.id)) byId.set(command.id, normalizeCommand(command, type));
    }
