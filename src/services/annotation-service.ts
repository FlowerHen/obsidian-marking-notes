...
interface FollowUpInput {
    ...
    settings: Pick<
        MarkingNoteSettings,
        'defaultSummarySystemPromptTemplate' | 'annotationSystemPromptTemplate' | 'tavilyApiKey'
    >;
    options?: {
...
export class AnnotationService {
    ...
    async rewriteSelection(input: InlineRewriteInput): Promise<boolean> {
        ...
        try {
            ...
            let newText = data.choices?.[0]?.message?.content?.trim() || input.selection;
    ...
    async annotateSelection(input: AnnotateSelectionInput): Promise<{ id: string; summary: string } | null> {
        const { from, to } = input.view.state.selection.main;
    ...
    private promptTemplates(settings: Pick<MarkingNoteSettings, 'defaultSummarySystemPromptTemplate' | 'annotationSystemPromptTemplate'>): PromptTemplates {
        return {
    ...
    private async buildSearchContext(input: FollowUpInput, nodeText: string): Promise<string | undefined> {
