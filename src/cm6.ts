...
class CapsuleWidget extends WidgetType {
    constructor(public node: MarkingNode, public tags: MarkingTag[]) {
        super();
    }
...
function buildDecorations(text: string, tags: MarkingTag[]): { decos: DecorationSet; nodes: MarkingNode[] } {
    ...
    for (const node of nodes) {
        ...
        const highlightDeco = Decoration.mark({ class: cssClass, attributes: attrs });
...
export function createMarkingExtensions(
    ...
    const mainPlugin = ViewPlugin.fromClass(class {
